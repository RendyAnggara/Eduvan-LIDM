                ...
                <tbody class="divide-y divide-gray-50">
                    @foreach ($course->users as $student)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4">
                                <span
                                    class="px-3 py-1 rounded-full text-[10px] font-bold bg-green-100 text-green-600 uppercase">
                                    Aktif
                                </span>
                            </td>
                            <!-- Perbaikan Nilai Quiz: Memakai data asli dari Controller -->
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center space-x-3">
                                    <div class="w-full bg-gray-100 rounded-full h-2 max-w-[120px]">
                                        <!-- Perbaikan Progress Bar: Lebar bar mengikuti persentase asli -->
                                        <div class="bg-green-500 h-2 rounded-full"
                                            style="width: {{ $student->persentase_asli }}%"></div>
                                    </div>
                                    <!-- Perbaikan Teks Persentase: Memakai data asli dari Controller -->
                                    <span class="text-xs font-bold text-gray-600">{{ $student->persentase_asli }}%</span>
                                </div>
                            </td>
                        </tr>
                    @endforeach
