...
@section('content')
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6">
        <div class="p-5 border-b border-gray-100 bg-amber-50/40">
            ...
            <p class="text-gray-500 text-xs mt-1">Daftar pendaftaran kelas mahasiswa yang menunggu konfirmasi bukti
                transfer.</p>
        </div>
                ...
                <tbody class="text-gray-600 text-sm divide-y divide-gray-100">
                    @forelse ($pendingVerifications as $verify)
                        <tr class="hover:bg-amber-50/20 transition-colors">
                            {{-- 🟢 FIX 1: Ditambahkan timezone Asia/Jakarta pada tabel antrean laptop --}}
                            <td class="p-4 text-gray-500 whitespace-nowrap">
                                {{ $verify->created_at->timezone('Asia/Jakarta')->format('d M Y, H:i') }}</td>
                ...
                <div class="bg-white p-4 rounded-xl border border-amber-100 shadow-sm space-y-3 relative">
                    <div class="flex justify-between items-start">
                        <div class="max-w-[70%]">
                            ...
                            {{-- 🟢 FIX 2: Ditambahkan timezone Asia/Jakarta pada kartu antrean mobile --}}
                            <span
                                class="text-[10px] text-gray-400 block mt-0.5">{{ $verify->created_at->timezone('Asia/Jakarta')->format('d M Y, H:i') }}</span>
                        </div>
                    ...
                    <div class="pt-2 border-t border-gray-50 space-y-2">
                        ...
                        <div class="flex justify-between items-center pt-1">
                            ...
                            @if ($verify->proof_of_payment)
                                <button type="button" onclick="openProofModal('{{ asset($verify->proof_of_payment) }}')"
                ...
                <tbody class="text-gray-600 text-sm divide-y divide-gray-100">
                    @forelse ($transactionDetails as $trans)
                        <tr class="hover:bg-gray-50/80 transition-colors">
                            {{-- 🟢 FIX 3: Ditambahkan timezone Asia/Jakarta pada tabel riwayat transaksi laptop --}}
                            <td class="p-4 text-gray-500 whitespace-nowrap">
                                {{ $trans->created_at->timezone('Asia/Jakarta')->format('d M Y, H:i') }}
                            </td>
                ...
                <div class="bg-white p-4 rounded-xl border border-gray-100 shadow-sm space-y-3 relative mobile-card">
                    <div class="flex justify-between items-start">
                        <div class="max-w-[70%]">
                            ...
                            {{-- 🟢 FIX 4: Ditambahkan timezone Asia/Jakarta pada kartu riwayat transaksi mobile --}}
                            <span
                                class="text-[10px] text-gray-400 block mt-0.5">{{ $trans->created_at->timezone('Asia/Jakarta')->format('d M Y, H:i') }}</span>
                        </div>
                        <span
