    ...
    <div class="px-2 sm:px-4 md:px-0 space-y-4 sm:space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div class="flex items-center justify-between sm:justify-start w-full sm:w-auto gap-2">
                <a href="{{ route('admin.students.index') }}"
                    class="inline-flex items-center gap-1.5 text-xs font-bold text-gray-600 hover:text-indigo-600 bg-white border border-gray-200 px-3 py-2 rounded-xl shadow-sm transition-all active:scale-95">
                ...
                <form action="{{ route('admin.students.destroy', $student->id) }}" method="POST" class="sm:hidden"
                    ...
                    @method('DELETE')
                    <button type="submit"
                        class="bg-rose-50 hover:bg-rose-100 text-rose-600 px-3 py-2 rounded-xl text-xs font-bold inline-flex items-center gap-1 transition active:scale-95">
            ...
            <div class="flex items-center justify-between sm:justify-end gap-2 w-full sm:w-auto">
                ...
                <form action="{{ route('admin.students.destroy', $student->id) }}" method="POST" class="hidden sm:block"
                    ...
                    @method('DELETE')
                    <button type="submit"
                        class="bg-rose-50 hover:bg-rose-100 text-rose-600 px-4 py-2.5 rounded-xl text-xs font-bold inline-flex items-center gap-1.5 transition cursor-pointer">
            ...
            <div
                ...
                <div class="flex-1 min-w-0 w-full">
                    <div class="flex flex-wrap items-center justify-center sm:justify-start gap-2">
                        <h2 class="text-xl sm:text-3xl font-black tracking-tight leading-tight">{{ $student->name }}</h2>
                        @if ($totalEnrollments > 0)
                            <span
                    ...
                    <div class="flex flex-wrap items-center justify-center sm:justify-start gap-1.5 sm:gap-2 mt-3 sm:mt-4">
                        @if ($student->school)
                            <span
                                class="inline-flex items-center px-2.5 py-1 rounded-xl text-[11px] sm:text-xs font-bold bg-white/10 border border-white/20 text-white backdrop-blur-sm max-w-full truncate">
                                <i class="fas fa-school text-[9px] mr-1.5 text-indigo-300"></i>
                                <span class="truncate">{{ $student->school->name }}</span>
                            </span>
                        @else
                            <span
                                class="inline-flex items-center px-2.5 py-1 rounded-xl text-[11px] sm:text-xs font-medium bg-white/10 border border-white/15 text-indigo-200 backdrop-blur-sm">
                        ...
                        @if ($studentClass)
                            <span
                                class="inline-flex items-center px-2.5 py-1 rounded-xl text-[11px] sm:text-xs font-bold bg-amber-400/20 border border-amber-300/30 text-amber-200 backdrop-blur-sm shrink-0">
                                <i class="fas fa-graduation-cap text-[9px] mr-1.5 text-amber-300"></i> Kelas
                                {{ $studentClass }} SMP
                            </span>
                        @else
                            <span
                                class="inline-flex items-center px-2.5 py-1 rounded-xl text-[11px] sm:text-xs font-medium bg-white/10 border border-white/15 text-indigo-200 backdrop-blur-sm shrink-0">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-4">
            <div
                ...
                <div
                    class="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center text-xs sm:text-sm font-bold shrink-0">
                    <i class="fas fa-book-open"></i>
                </div>
                <div class="min-w-0">
                    <p class="text-[9px] sm:text-[11px] font-bold text-gray-400 uppercase tracking-wider truncate">Total
                        Course</p>
                    <h3 class="text-base sm:text-lg font-black text-gray-900 leading-none mt-0.5 sm:mt-1">
                        {{ $totalEnrollments }}</h3>
                </div>
            </div>
            ...
            <div
                class="bg-white p-3.5 sm:p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-2.5 sm:gap-3">
                <div
                    class="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center text-xs sm:text-sm font-bold shrink-0">
                    <i class="fas fa-chart-line"></i>
                </div>
                <div class="min-w-0">
                    <p class="text-[9px] sm:text-[11px] font-bold text-gray-400 uppercase tracking-wider truncate">Rata-Rata
                        Progres</p>
                    <h3 class="text-base sm:text-lg font-black text-emerald-600 leading-none mt-0.5 sm:mt-1">
                        {{ $avgProgress }}%</h3>
                </div>
            </div>
            ...
            <div
                class="bg-white p-3.5 sm:p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-2.5 sm:gap-3">
                <div
                    class="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center text-xs sm:text-sm font-bold shrink-0">
                    <i class="fas fa-id-card"></i>
                </div>
                <div class="min-w-0">
                    <p class="text-[9px] sm:text-[11px] font-bold text-gray-400 uppercase tracking-wider truncate">NISN /
                        NIP</p>
                    <h3 class="text-xs sm:text-sm font-black text-gray-800 leading-none mt-0.5 sm:mt-1 truncate">
                        {{ $student->nisn_or_nip ?? '-' }}</h3>
                </div>
            </div>
            ...
            <div
                class="bg-white p-3.5 sm:p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-2.5 sm:gap-3">
                <div
                    class="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-purple-50 text-purple-600 flex items-center justify-center text-xs sm:text-sm font-bold shrink-0">
                    <i class="fas fa-calendar-alt"></i>
                </div>
                <div class="min-w-0">
                    <p class="text-[9px] sm:text-[11px] font-bold text-gray-400 uppercase tracking-wider truncate">Terdaftar
                        Pada</p>
                    <h3 class="text-[11px] sm:text-xs font-black text-gray-800 leading-none mt-0.5 sm:mt-1 truncate">
                        {{ $student->created_at->format('d M Y') }}</h3>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-4 sm:space-y-6">
            ...
            @else
                <div class="bg-white p-8 sm:p-14 rounded-3xl shadow-sm text-center border border-gray-100 space-y-3">
                    <div
